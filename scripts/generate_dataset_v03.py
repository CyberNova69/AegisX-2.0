#!/usr/bin/env python3
"""
AegisX Synthetic SOC Dataset Generator (v0.3)
==============================================

Generates AegisX Fine-Tuning Dataset v0.3:
- 85 genuinely distinct template families across 12 security domains
- Balanced class distribution across 5 classifications (~20% each)
- 42 deliberate borderline scenarios across all 5 key decision boundaries
- All 5 severity levels (informational, low, medium, high, critical)
- Both Windows (70) and Linux (15) endpoint telemetry
- 16 of 18 evidence types
- 33 MITRE ATT&CK techniques across all 10 tactics
- Anti-keyword learning design (key tools like PowerShell, certutil, rundll32 appear across multiple classes)
- Template-stratified train/validation/test split (80/10/10) with zero template leakage

Usage:
    python scripts/generate_dataset_v03.py --count 520 --seed 42
"""
...
HOSTNAMES_LINUX = [
    ...
    "PROD-API-01", "SRV-K8S-NODE-01", "DB-POSTGRES-01"
]

USERNAMES_STANDARD = [
    "employee01", "employee02", "employee05", "employee10", "contractor01", "intern01",
    "developer01", "analyst02", "accountant01"
]
USERNAMES_ADMIN = ["admin01", "admin02", "it_admin01", "it_admin02", "sysadmin01"]
USERNAMES_SERVICE = [
    "svc_backup", "svc_monitor", "svc_deploy", "svc_scanner", "svc_sql", "svc_sccm"
]
USERNAMES_LINUX = ["ubuntu", "root", "deployer", "devops", "www-data", "secops", "jenkins"]
...
EXTERNAL_IPS = [
    ...
    "194.26.29.112", "185.190.140.22", "195.123.245.8"
]
...
# ---------------------------------------------------------------------------
# Generator Engine with Template Stratified Split
# ---------------------------------------------------------------------------

def generate_finetuning_v03(output_dir: Path, target_count: int = 520, seed: int = 42) -> dict:
    import templates_v03

    rng = random.Random(seed)
    gen = SyntheticValueGenerator(rng)

    templates = templates_v03.TEMPLATES
    ...
    print(f"  Train templates:      {len(train_templates)} (80.0%)")
    print(f"  Validation templates: {len(val_templates)} (10.6%)")
    print(f"  Test templates:       {len(test_templates)} (9.4%)")

    # Allocation of records per split: 80% train (~416), 10% val (54), 10% test (50) -> total 520
    # In train: 68 templates * 6 = 408; we add 8 extra to hit 416
    # In val: 9 templates * 6 = 54
    # In test: 8 templates * 6 = 48; we add 2 extra to hit 50
    # Total = 416 + 54 + 50 = 520 records

    split_quotas = {
        "train": (train_templates, 416),
        "val": (val_templates, 54),
        "test": (test_templates, 50),
    }
    ...
    for split_name, (t_list, target_split_count) in split_quotas.items():
        base_per_template = target_split_count // len(t_list)
        remainder = target_split_count % len(t_list)

        # Distribute remainder across templates deterministically
        counts_per_template = [base_per_template + (1 if i < remainder else 0) for i in range(len(t_list))]

        # Generate instances
        instances = []
        for t, count in zip(t_list, counts_per_template):
            for _ in range(count):
                instances.append(t)
        ...
        for t in instances:
            ...
            rec["metadata"] = {
                "source": "synthetic",
                "generator": "aegisx-v0.3-generator",
    ...
    # 1. Write individual split files
    ...
    print(f"Wrote template inventory to {inv_path.name}")
    ...
    config_yaml = f"""# AegisX Fine-Tuning Dataset Generation Configuration v0.3
dataset:
  name: "aegisx-soc-finetuning"
  version: "0.3"
  description: "Production fine-tuning dataset with template-stratified split and anti-keyword design"
...
"""
    ...
    card_md = f"""# AegisX SOC Fine-Tuning Dataset Card (v0.3)

## Dataset Summary
- **Name**: aegisx-soc-finetuning
- **Version**: 0.3
- **Total Records**: {len(all_records)}
- **Template Families**: {len(templates)}
- **Generator**: `scripts/generate_dataset_v03.py` (seed {seed})
...
|---|---|---|---|
| **Train** | {len(records_by_split["train"])} | 68 (80.0%) | Supervised training set |
| **Validation** | {len(records_by_split["val"])} | 9 (10.6%) | Validation checkpoint evaluation |
| **Test** | {len(records_by_split["test"])} | 8 (9.4%) | Held-out unseen template generalization benchmark |
...
## Borderline Cases
- Deliberate borderline records: {sum(1 for r in all_records if "borderline_category" in r["metadata"])}
...
def main():
    parser = argparse.ArgumentParser(description="AegisX Synthetic SOC Dataset Generator (v0.3)")
    parser.add_argument("--count", type=int, default=520, help="Total number of examples to generate")
    parser.add_argument("--output-dir", type=str, default="datasets/finetuning/v0.3", help="Output directory path")
    ...
    print("============================================================")
    print("  AegisX Fine-Tuning Dataset Generator (v0.3)")
    print("============================================================")
    print(f"  Target Count: {args.count}")
    print(f"  Seed:         {args.seed}")
    print(f"  Output Dir:   {out_dir}\n")

    meta = generate_finetuning_v03(out_dir, target_count=args.count, seed=args.seed)
